...
This post gives you a basic guide for integrating Azure AD with your 1Password Business to automate 1Password user management based on your AD. Integration requires deploying service called [SCIM bridge](http://www.simplecloud.info), which basically ensures integration of Azure AD with 1Password using SCIM v2 standard (well, apparently that's why it's called bridge). Basic requirements and short tutorial of setup is provided on a [ 1Password's site](https://support.1password.com/scim-azure-ad/) so this post will be more like a summary of all the other actions involved to ensure the integration service running. This is more kind of summary about the first experience with the Kubernetes.
